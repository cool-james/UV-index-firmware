...
// mbed OS uses 8 bit addresses, must bit shift
#define SI1145_ADDR 0x60 << 1

/// *** For calibration (taken from silabs example code) *** ///
...
class Adafruit_SI1145  {
 ...
 private:
  void readBlock(uint8_t a, char* buffer, uint8_t num_reads);
  uint16_t read16(uint8_t addr);
  uint8_t read8(uint8_t addr);
  void write8(uint8_t reg, uint8_t val);
  void write32(uint8_t reg, char* buf);
  uint8_t readParam(uint8_t p);
  uint8_t writeParam(uint8_t p, uint8_t v);
  void si114x_set_ucoef( uint8_t* input_ucoef, SI114X_CAL_S* si114x_cal );
